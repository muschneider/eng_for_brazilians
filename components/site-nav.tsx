  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-[var(--line-soft)] bg-[color-mix(in_oklab,var(--bg)_82%,transparent)] backdrop-blur-xl">
        <div className="mx-auto flex h-14 w-full max-w-6xl items-center gap-2 px-4 sm:px-6">
            ...
            {LINKS.map(({ href, label, Icon, exact }) => {
              ...
              return (
                <Link
                  ...
                  className="relative flex items-center gap-1.5 rounded-[var(--radius-md)] px-3 py-2 text-[0.875rem] font-medium transition-colors"
                  style={{
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            >
              ...
              <kbd className="en hidden rounded border border-[var(--line)] px-1 text-[0.6875rem] lg:inline">
                ⌘K
              </kbd>
            </button>

            {ready && (
              <div className="hidden items-center gap-1.5 rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--surface-2)] px-2.5 py-1.5 sm:flex">
                <IconFlame
                  className="h-4 w-4"
                  style={{
                    color:
                ...
                <span className="ml-0.5 hidden text-[0.75rem] text-[var(--fg-faint)] lg:inline">
                  Nv.{level.level}
                </span>
              </div>
            )}
            ...
            <button
              ...
              aria-label="Menu"
              aria-expanded={open}
              className="grid h-9 w-9 place-items-center rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--surface-2)] text-[var(--fg-muted)] md:hidden"
            >
              {open ? (
        ...
        {open && (
          <nav className="border-t border-[var(--line-soft)] bg-[var(--surface)] px-4 py-2 md:hidden">
            {LINKS.map(({ href, label, Icon, exact }) => {
              const active = isActive(href, exact);
              return (
                <Link
